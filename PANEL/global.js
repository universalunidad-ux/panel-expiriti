...
export const initAppRail=page=>{const rail=$("#appRail"),scrim=$("#appScrim"),toggle=$("#railToggle");if(!rail)return;toggle?.addEventListener("click",()=>{rail.classList.toggle("open");scrim?.classList.toggle("open",rail.classList.contains("open"))});scrim?.addEventListener("click",()=>{rail.classList.remove("open");scrim.classList.remove("open")});document.querySelectorAll("[data-rail-parent]").forEach(b=>b.addEventListener("click",()=>b.closest(".rail-group")?.classList.toggle("open")));if(page){document.querySelector(`[data-nav="${page}"]`)?.classList.add("is-active")}};
