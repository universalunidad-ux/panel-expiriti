...
const panelHtml=()=>`<div class="app-panel-backdrop" id="appPanelBackdrop"></div><aside class="app-panel" id="appPanel"><div class="app-panel-head"><div><div class="app-panel-title" id="appPanelTitle">Panel</div><div class="app-panel-sub" id="appPanelSub">Vista interna</div></div><button class="close-x" id="appPanelClose" type="button">×</button></div><div class="app-panel-body" id="appPanelBody"></div></aside>`;
...
export const ensureAppShell=({page,title="",kicker="",actionsHtml="",role="soporte"}={})=>{if($("#appRail")&&$("#appPanel")&&$(".topbar"))return;const shell=$("#appShell");if(!shell)return;const rk=roleKey(role);shell.innerHTML=`${railHtml(rk,page)}${panelHtml()}${topbarHtml({page,title,kicker,actionsHtml})}`;initAppRail(page);initAppPanel();setAppRole(rk);bindGlobalSearch()};
...
export const initAppPanel=()=>{if(document.documentElement.dataset.appPanelBound)return;document.documentElement.dataset.appPanelBound="1";document.addEventListener("click",e=>{if(e.target.closest("#appPanelClose")||e.target.closest("#appPanelBackdrop"))closeAppPanel();const op=e.target.closest("[data-open-panel]");if(!op)return;const key=op.dataset.openPanel;if(key==="recent_clients"){const items=readRecentClients(),html=items.length?`<div class="panel-card"><div class="panel-kv"><div class="panel-k">Últimos clientes vistos</div><div class="panel-v">${items.map(x=>`<button class="btn" type="button" data-recent-client="${esc(x.id)}" data-recent-name="${esc(x.nombre)}">${esc(x.nombre)}</button>`).join(" ")}</div></div></div>`:`<div class="panel-card"><div class="panel-kv"><div class="panel-k">Últimos clientes vistos</div><div class="panel-v">Aún no hay clientes recientes.</div></div></div>`;return openAppPanel({title:"Clientes recientes",sub:"Últimos clientes vistos",html})}const cfg=APP_PANEL_MAP[key];if(cfg)openAppPanel(cfg)});document.addEventListener("keydown",e=>{const tag=(document.activeElement?.tagName||"").toLowerCase(),typing=["input","textarea","select"].includes(tag)||document.activeElement?.isContentEditable;if(e.key==="Escape")return closeAppPanel();if(typing&&e.key!=="/")return;if(e.key==="?"){e.preventDefault();return openAppPanel(APP_PANEL_MAP.shortcuts)}if(e.key==="/"){e.preventDefault();return document.querySelector("#searchInput,#tkSearch")?.focus()}if(e.key.toLowerCase()==="i"&&window.__exp_next_key==="d"){window.__exp_next_key="";location.href="dashboard.html";return}if(e.key.toLowerCase()==="i"&&window.__exp_next_key==="t"){window.__exp_next_key="";location.href="tickets.html";return}if(e.key.toLowerCase()==="n"&&window.__exp_next_key==="t"){window.__exp_next_key="";document.querySelector("#heroNewTicketBtn,#newTicketTopBtn,#tkNewBtn")?.click();return}if(e.key.toLowerCase()==="i"){window.__exp_next_key="";setTimeout(()=>window.__exp_next_key="",700);window.__exp_next_key="pending_i";return}if(e.key.toLowerCase()==="d"&&window.__exp_next_key==="pending_i"){window.__exp_next_key="d";return}if(e.key.toLowerCase()==="t"&&window.__exp_next_key==="pending_i"){window.__exp_next_key="t";return}if(e.key.toLowerCase()==="n"){window.__exp_next_key="";setTimeout(()=>window.__exp_next_key="",700);window.__exp_next_key="pending_n";return}if(e.key.toLowerCase()==="t"&&window.__exp_next_key==="pending_n"){window.__exp_next_key="t";return}});document.addEventListener("click",e=>{const b=e.target.closest("[data-recent-client]");if(!b)return;location.href=`cliente.html?id=${b.dataset.recentClient}`})};
...
export const initAppRail=page=>{const rail=$("#appRail"),scrim=$("#appScrim"),toggle=$("#railToggle");if(!rail)return;const mobile=()=>window.matchMedia("(max-width: 980px)").matches;toggle?.addEventListener("click",()=>{rail.classList.toggle("open");scrim?.classList.toggle("open",rail.classList.contains("open"))});scrim?.addEventListener("click",()=>{rail.classList.remove("open");scrim.classList.remove("open")});document.querySelectorAll("[data-rail-parent]").forEach(b=>{b.addEventListener("click",e=>{if(!mobile())return;e.preventDefault();b.closest(".rail-group")?.classList.toggle("open")})});if(page)document.querySelector(`[data-nav="${page}"]`)?.classList.add("is-active")};
