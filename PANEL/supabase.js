...
export async function getProfile(){const{data:{user}}=await supabase.auth.getUser();if(!user)return null;let{data}=await supabase.from("perfiles").select("id,nombre,rol,tema,preferencias").eq("id",user.id).maybeSingle();if(data)return data;const ins=await supabase.from("perfiles").insert({id:user.id,tema:"system",rol:"soporte"}).select("id,nombre,rol,tema,preferencias").single();return ins.data||null}
export async function saveTheme(v,id){if(!id)return;const q=await supabase.from("perfiles").update({tema:v}).eq("id",id);if(q.error&&/0 rows|No rows/i.test(q.error.message||""))await supabase.from("perfiles").insert({id,tema:v,rol:"soporte"})}
...
export function needsProfileRefresh(p){const miss=!p?.nombre||!p?.telefono||!p?.correo,last=+new Date(p?.datos_confirmados_en||0)||0;return miss||!last||Date.now()-last>PROFILE_RECHECK_MS}
