...
async function idemStart(key:string,action:string,resource_id:string,request_hash:string){if(!key)return{mode:"none"};const now=new Date().toISOString(),row={idempotency_key:key,action,resource_id,request_hash,status:"processing",created_at:now,updated_at:now};const ins=await sb.from("edge_idempotency").insert({idempotency_key:key,action,resource_id,request_hash,status:"processing"});if(!ins.error)return{mode:"new",row};if(ins.error.code!=="23505")throw new Error(`IDEMPOTENCY_INSERT_ERROR: ${ins.error.message}`);const old=await sb.from("edge_idempotency").select("*").eq("idempotency_key",key).maybeSingle();if(old.error)throw new Error(`IDEMPOTENCY_READ_ERROR: ${old.error.message}`);if(!old.data)return{mode:"new",row};if(old.data.request_hash&&old.data.request_hash!==request_hash)return{mode:"conflict",row:old.data};if(old.data.status==="completed"&&old.data.response)return{mode:"replay",row:old.data};const age=Date.now()-new Date(old.data.updated_at||old.data.created_at||0).getTime();if(old.data.status==="failed"||age>90000){const reset=await sb.from("edge_idempotency").update({status:"processing",error:null,updated_at:now}).eq("idempotency_key",key);if(reset.error)throw new Error(`IDEMPOTENCY_RESET_ERROR: ${reset.error.message}`);return{mode:"retry",row:{...old.data,status:"processing",error:null,updated_at:now}}}return{mode:"processing",row:old.data}}
...
const idem=await withTimeout(idemStart(idemKey,"ticket-internal-reply",ticket_id,requestHash),9000,"IDEMPOTENCY_START");
